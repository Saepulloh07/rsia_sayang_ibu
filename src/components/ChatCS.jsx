...
const ChatCS = () => {
  ...
  const [messages, setMessages] = useState([
    { sender: 'CS', text: 'Selamat datang di Layanan Chat RSIA Sayang Ibu. Bagaimana kami bisa membantu Anda hari ini? (Contoh: pertanyaan tentang layanan, janji temu, atau keluhan kesehatan ibu/anak)' },
  ]);
  ...
  const handleSend = () => {
    if (newMessage.trim()) {
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          { sender: 'CS', text: 'Terima kasih atas pesan Anda. Tim kami akan segera merespons. Untuk urgensi, hubungi IGD: +62 123 456 7890.' },
        ]);
      }, 1000);
      setNewMessage('');
    }
    ...
    <>
      ...
      <Box
        ...
        sx={{
          ...
          border: isMobile ? 'none' : `2px solid ${scrolled ? '#4CAF50' : '#FFFFFF'}`,
      ...
      >
        ...
        >
          ...
          <Typography
            id="chat-modal-title"
            variant="h6"
            sx={{ fontWeight: 600, mb: 2, color: '#4CAF50', textAlign: 'center' }}
          >
          ...
          <List sx={{ flexGrow: 1, overflowY: 'auto', mb: 2 }}>
            {messages.map((msg, index) => (
              <ListItem key={index} sx={{ justifyContent: msg.sender === 'Pasien' ? 'flex-end' : 'flex-start' }}>
                <ListItemText
                  primary={msg.text}
                  sx={{
                    backgroundColor: msg.sender === 'Pasien' ? '#E8F5E9' : '#F5F5F5',
                    p: 1.5,
                    borderRadius: 2,
                    maxWidth: '80%',
                  }}
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <TextField
              ...
              sx={{ mr: 1 }}
            />
            <IconButton color="primary" onClick={handleSend} aria-label="Kirim Pesan">
              <SendIcon />
            </IconButton>
          </Box>
